...
from .utils import latest, module_summary, result, safe_div, score_higher_better, score_lower_better


def current_ratio(df: pd.DataFrame, benchmarks: dict) -> dict:
    values = (df["current_assets"] / df["current_liabilities"]).replace([np.inf, -np.inf], 0).fillna(0).tolist()
...
def quick_ratio(df: pd.DataFrame, benchmarks: dict) -> dict:
    values = ((df["current_assets"] - df["inventory"]) / df["current_liabilities"]).replace([np.inf, -np.inf], 0).fillna(0).tolist()
...
def interest_coverage(df: pd.DataFrame, benchmarks: dict) -> dict:
    values = (df["ebit"] / df["interest_expense"]).replace([np.inf, -np.inf], 0).fillna(0).tolist()
    flags = []
    if latest(values) < 2:
...
def dscr(df: pd.DataFrame, benchmarks: dict) -> dict:
    ...
    values = (df["operating_income"] / service).replace([np.inf, -np.inf], 0).fillna(0).tolist()
...
def cash_debt_coverage(df: pd.DataFrame, benchmarks: dict) -> dict:
    values = (df["operating_cash_flow"] / df["total_debt"]).replace([np.inf, -np.inf], 0).fillna(0).tolist()
...
def net_debt_to_ebitda(df: pd.DataFrame, benchmarks: dict) -> dict:
    values = ((df["total_debt"] - df["cash"]) / df["ebitda"]).replace([np.inf, -np.inf], 0).fillna(0).tolist()
...
def debt_ratio(df: pd.DataFrame, benchmarks: dict) -> dict:
    values = (df["total_debt"] / df["total_assets"]).replace([np.inf, -np.inf], 0).fillna(0).tolist()
...
def long_term_debt_cap(df: pd.DataFrame, benchmarks: dict) -> dict:
    values = (df["long_term_debt"] / (df["long_term_debt"] + df["equity"])).replace([np.inf, -np.inf], 0).fillna(0).tolist()
...
def goodwill_assets(df: pd.DataFrame, benchmarks: dict) -> dict:
    values = (df["goodwill"] / df["total_assets"] * 100).replace([np.inf, -np.inf], 0).fillna(0).tolist()
