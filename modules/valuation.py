...
from .utils import latest, module_summary, result, safe_div, score_lower_better


def ps_ratio(df: pd.DataFrame, benchmarks: dict, market_cap: float) -> dict:
    value = safe_div(market_cap, latest(df["revenue"].tolist()))
...
def ev_ebitda(df: pd.DataFrame, benchmarks: dict, market_cap: float) -> dict:
    ...
    value = safe_div(enterprise_value, latest(df["ebitda"].tolist()))
...
def pb_ratio(df: pd.DataFrame, benchmarks: dict, market_cap: float) -> dict:
    value = safe_div(market_cap, latest(df["equity"].tolist()))
    roe = safe_div(latest(df["net_profit"].tolist()), latest(df["equity"].tolist())) * 100
